    ...
    useEffect(() => {
        ...
        gsap.timeline({
          ...
          scrollTrigger: {
            ...
            pin: grid.parentNode,
            scrub: 0.3,
            toggleActions: "play reverse play reverse", // Enables forward & reverse animations
          },
        })
          .to(overlay, { opacity: 0, duration: 0.02 }, 0) // Overlay fade out
          .fromTo(
            gridImages,
            {
              x: 0,
...
<div className={styles.grid + " " + styles.gridSmall} data-grid-fourth-v2 style={{ perspective: "1200px" }}>
{[...Array(20)].map((_, i) => {
  return (
    <div key={i} className={styles.grid__img} 
         style={{ backgroundImage: `url(/assets/thumb-${i + 1}.jpg)`, backgroundSize: "cover", backgroundPosition: "center" }}>
    </div>
  );
})}
      ...
      <div className={styles.bannerContent}>
        <h1 className={`${styles.bannerText} heading-1`}>We are a Creative powerhouse
        for ambitious Brands</h1>
      </div>
    </section>
  );
};
